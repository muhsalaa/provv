  ...
  if (skillArgs.length > 0) {
    ...
    if (isCommand) {
      ...
      if (selectedSkills.length < skillArgs.length) {
        const missing = skillArgs.filter((a) => !selectedSkills.some((s) => s.name === a));
    ...
    if (options.length <= 1) {
      ...
      const updatedSkills = getAllSkills(masterPath);
      selectedSkills = updatedSkills.filter((s) => installedNames.includes(s.name));
    } else {
    ...
    try {
      ...
      if (!inMaster) {
        ...
        if (gitExclude === "auto-ignore") {
          ...
          perSkillRollback.push(() => {
            try {
              removeFromGitExclude(targetProject, `.agents/skills/${skill.name}`);
            } catch {
              /* best-effort */
            }
          });
